...
export default function AdminPanel({ token, onLogout }) {
    ...
    try {
      ...
      const response = await fetch('/api/admin/candidates', {
        ...
        headers: {
          ...
          // Note: DO NOT set 'Content-Type' when sending FormData. The browser sets it with boundaries automatically.
        },
        body: formData
      });
  ...
  const handleResetElection = async () => {
    if (!window.confirm('🚨 WARNING: This will delete ALL cast votes and custom photo uploads! Are you absolutely sure?')) return;
    setError('');
    setActionSuccess('');
    ...
    try {
      ...
      setActionSuccess('Election successfully reset. All student statuses set to "Not Voted".');
      fetchStats();
    } catch (err) {
      setError(err.message);
    }
  ...
  return (
    <div className="animate-fade-in" style={{ width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-secondary)' }}>
            Real-time voter tracking, voter unblocks, candidate management, and election configuration.
          </p>
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <button className="btn btn-secondary" onClick={fetchStats} title="Refresh Live Data" style={{ padding: '0.75rem' }}>
            <RefreshCw size={16} />
          </button>
          <button className="btn btn-secondary" onClick={handleResetElection} style={{ border: '1px solid var(--color-danger)', color: '#f87171' }}>
            Reset Election
          </button>
      ...
      <div className="glass-panel" style={{ padding: '1.75rem 2rem', marginBottom: '2.5rem', background: 'rgba(239, 68, 68, 0.02)' }}>
        <h2 style={{ fontSize: '1.2rem', fontWeight: '700', color: '#f87171', display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              ...
              <tbody>
                {stats?.blockedVoters.map(voter => (
                  <tr key={voter.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ padding: '0.75rem 0.5rem', fontWeight: 'bold', color: 'white' }}>{voter.id}</td>
                    <td style={{ padding: '0.75rem 0.5rem' }}>{voter.name}</td>
                    <td style={{ padding: '0.75rem 0.5rem' }}>{voter.email}</td>
                    <td style={{ padding: '0.75rem 0.5rem', color: 'var(--color-danger)' }}>{voter.failed_attempts} attempts</td>
        ...
        <div>
          <div className="glass-panel" style={{ padding: '2rem' }}>
            <h2 className="header-title" style={{ fontSize: '1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <BarChart3 size={20} style={{ color: 'var(--color-primary)' }} /> Live Standing Tally
            </h2>
